...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener(async (message, sender) => {
    ...
    if (message.action === 'conversationContent' && message.conversationId) {
      console.log(
        `Background: Received content update for ${message.conversationId}`
      );
    ...
    if (
      message.action === 'conversationInitialContent' &&
      message.conversationId
    ) {
      console.log(
        `Background: Received INITIAL content for ${message.conversationId}`
      );
    ...
    if (message.action === 'conversationNavigated' && message.conversationId) {
      console.log(
        `Background: User navigated to conversation ${message.conversationId}`
      );
